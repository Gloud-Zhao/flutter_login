...
class LoginCallback {
  ...
  String accountValidator(String value) => null;
...
List<LoginData> stubCallback(MockCallback mockCallback) {
  ...
  when(mockCallback.accountValidator(user.name)).thenReturn(null);
  when(mockCallback.accountValidator('invalid-name')).thenReturn('Invalid!');
